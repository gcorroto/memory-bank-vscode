...
async function loadProjectConfig(projectId: string): Promise<{ sourcePath?: string } | null> {
  try {
    ...
    log(`[FrameworkDetector] Looking for metadata at: ${metadataPath}`);
    
    if (fs.existsSync(metadataPath)) {
      log(`[FrameworkDetector] Found metadata.json`);
      ...
      log(`[FrameworkDetector] metadata.json found but no sourcePath field`);
    }
    ...
    if (fs.existsSync(registryPath)) {
      ...
      if (project?.path) {
        log(`[FrameworkDetector] Found project path in global_registry: ${project.path}`);
        return { sourcePath: project.path };
      }
    }
    
    log(`[FrameworkDetector] No sourcePath found for project ${projectId}`);
    return null;
  } catch (error) {
    log(`[FrameworkDetector] ERROR loading project config: ${error}`);
    return null;
  }
}
